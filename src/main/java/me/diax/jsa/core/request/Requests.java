...
import me.diax.jsa.core.StringUtils;
 ...
 *
 * This class contains all the valid request
 */
public class Requests {
    ...
    protected BaseRequest create(Request request, String... params) {
        ...
        if (!request.point.isEmpty()) {
            ...
            if (count < params.length || params.length > count)
                throw new IllegalArgumentException("Param count and %s count do nat match!");
            else url += String.format(request.point, (Object[]) params);
        }
        String body = request.body;
        if ((body == null || body.isEmpty() || !request.hasBody()) && request.requiresBody)
            throw new IllegalArgumentException("Request requires body.");
